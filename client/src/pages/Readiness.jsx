...
export default function Readiness() {
  ...
  const breakdownItems = [
    { label: 'Coding Accuracy', value: data?.breakdown?.codingAccuracy ?? 0, icon: <Code2 className="w-4 h-4" />, zeroText: 'Not attempted' },
    { label: 'Aptitude Score', value: data?.breakdown?.aptitudeScore ?? 0, icon: <BookOpen className="w-4 h-4" />, zeroText: 'Not attempted' },
    { label: 'ATS Score', value: data?.breakdown?.atsScore ?? 0, icon: <FileText className="w-4 h-4" />, zeroText: 'Not analyzed' },
    { label: 'Mock Interview', value: data?.breakdown?.mockInterviewScore ?? 0, icon: <Mic className="w-4 h-4" />, zeroText: 'Not attempted' },
  ];
          ...
          <div className="bg-gray-900 rounded-xl border border-gray-800 p-6 space-y-4">
            ...
            {breakdownItems.map((item) => (
              <div key={item.label} className="flex items-center justify-between">
                ...
                <span className={`font-bold ${item.value > 0 ? scoreColor(item.value) : 'text-gray-500'}`}>
                  {item.value > 0 ? item.value : item.zeroText}
                </span>
              </div>
            ))}
          </div>
