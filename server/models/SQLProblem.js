...
const SQLSchemaTableSchema = new mongoose.Schema({
  ...
  columns: [{ name: { type: String, required: true }, type: { type: String, required: true } }],
  notes: String,
}, { _id: false });

const SQLExampleSchema = new mongoose.Schema({
  exampleNumber: { type: Number, required: true },
  inputTables: [{ tableName: { type: String, required: true }, rows: [mongoose.Schema.Types.Mixed] }],
  outputTable: { columns: [String], rows: [mongoose.Schema.Types.Mixed] },
  explanation: String,
}, { _id: false });
...
const SQLProblemSchema = new mongoose.Schema({
  ...
  referenceSolutionSQL: { type: String, default: '' },
