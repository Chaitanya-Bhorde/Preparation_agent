...
exports.getCompanyInfo = async (req, res) => {
  try {
    ...
    const problemIds = await Problem.find({ companies: company, isActive: true }).distinct('_id');
    const acceptedCount = await Submission.countDocuments({ user: userId, status: 'accepted', problem: { $in: problemIds } });
